...
% Date:   Oct 2017
% Purpose:Phase II report, based on daily mileage data, 
...
time_period = '2010-07-01_to_2018-08-15';
file_inc = ls([folder '\Incidents_' time_period '.mat']);
load([folder '\' file_inc])
file_mileage = ls([folder '\Mileages_' time_period '*.mat']);
...
label_year = [num2str(year(datetime(datestr(Mileage.start_date)))+[0:years_inc-1]')];
label_quarter = char(unique(category_in_quarter));
%Trip Lost Service Interruption at 14th column
incident_intrpt_index = Incident.events_counted;
...
%h = histogram(incident_in_quarter,1:max(incident_in_quarter));
fleet_incident_counts_quarter = h.Values;
...
incident_intrpt_by_car = str2double(cellstr(Incident.Content(incident_intrpt_index,1)));
...
cars_oos = [3803 3807 3808 3832 3854 3873 3879 3891 3893]; %3836 taken off list
...
incident_intrpt_by_car_by_year = histcounts2(incident_intrpt_by_car, incident_intrpt_in_quarter,...
    [cars_with_intrpt_incidents-0.5;(cars_with_intrpt_incidents(end)+0.5)], [0.5:4:(quarters_inc+0.5)]  );

% to count overall incidents by car
incident_counts_by_car = zeros(units,1);
incident_counts_by_car(validcars) = histcounts(incident_intrpt_by_car,...
...
MMBF_quarterly = fleet_miles./fleet_incident_counts_quarter;
figure,bar(MMBF_quarterly,'FaceColor',[0 .5 .5],'EdgeColor',[0 .5 .5],'LineWidth',1.5)
yneg = MMBF_quarterly-fleet_miles./(fleet_incident_counts_quarter+2);
ypos = fleet_miles./(fleet_incident_counts_quarter-2)-MMBF_quarterly;
...
text(1:quarters_inc,MMBF_quarterly+250,num2str(MMBF_quarterly', '%.f Miles'),'HorizontalAlignment','center')
%% yearly arrangement

MMBF_yearly = sum(reshape([quarterfiller fleet_miles],4,years_inc),1)./sum(reshape([quarterfiller fleet_incident_counts_quarter],4,years_inc),1);
...
MMBF_car = sum(Mileage.Vals,2)./incident_counts_by_car;
...
%% MMBF by car by year
miles_car_year = squeeze(sum(reshape([repmat(quarterfiller',[units 1]) miles_by_car_by_quarter],units,4,years_inc),2));
%miles_car_year(miles_car_year<200) = 0;
MMBF_car_year = miles_car_year(validcars,:)./incident_intrpt_by_car_by_year;
...
MMBF_car_last7years = sum(miles_car_year(validcars,:),2)./sum(incident_intrpt_by_car_by_year,2);
...
for n=1:lastyears
    tmp = (lastyears-n);
    clear sorted*
    [sorted,sortedi]=sort(toplot(:,end-tmp),1);
